...
export const eventsTable = pgTable(
  "events",
  {
    ...
    startTime: varchar("startTime", { length: 13 }),
    endTime: varchar("endTime", { length: 13 }),
    userHandle: varchar("user_handle", { length: 50 })
      .notNull()
      // In this case, we want to delete the event if the user
      // is deleted, so we use onDelete: "cascade". It is similar for onUpdate.
      .references(() => usersTable.handle, {
        onDelete: "cascade",
        onUpdate: "cascade",
  ...
  (table) => ({
    ...
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    // uniqCombination: unique().on(table.userHandle, table.title), 
...
export const likesTable = pgTable( //join
  "likes",
  {
    id: serial("id").primaryKey(),
    userHandle: varchar("user_handle", { length: 50 })
